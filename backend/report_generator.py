...
class ReportPDF(FPDF):
    ...
    def header(self):
        ...
        self.set_y(10)
        self.set_x(12)
        self.cell(0, 10, self.DOC_TITLE, align="L", border=0)
        ...
        self.set_y(10)
        self.set_x(-75)
        self.cell(63, 10, datetime.utcnow().strftime("%Y-%m-%d  |  IDP-772"), align="R", border=0)
        self.ln(22)
    ...
    def footer(self):
        ...
        self.cell(0, 10, f"AI-Generated Intelligence Summary  ·  Page {self.page_no()}", align="C")
    ...
    def data_row(self, label: str, value: str):
        ...
        self.cell(45, 8, f" {label}:", border=0)
        self.set_font("Helvetica", "", 9)
        self.set_text_color(*self.C_TEXT)
        remaining_width = self.w - self.get_x() - self.r_margin
        self.multi_cell(remaining_width, 8, str(value), border=0)
        self.ln(1)
...
def generate_pdf_report(
    ...
    if annotations:
        ...
        for i, ann in enumerate(annotations, 1):
            ...
            pdf.cell(0, 6, f"ANNOTATION #{i} [{ann.get('timestamp', 'N/A')}]", ln=True)
            pdf.set_font("Helvetica", "", 10)
            pdf.set_text_color(*pdf.C_TEXT)
            pdf.set_fill_color(252, 252, 255)
            pdf.multi_cell(0, 6, ann["text"], border='L', fill=True)
            pdf.ln(2)
    ...
    if chat_history:
        ...
        for i, item in enumerate(chat_history, 1):
            ...
            pdf.multi_cell(0, 8, f"QUERY {i}: {item['query']}", fill=True)
            pdf.set_font("Helvetica", "", 10)
            pdf.set_text_color(*pdf.C_TEXT)
            pdf.ln(1)
            pdf.multi_cell(0, 6, item["response"])
            pdf.ln(4)
